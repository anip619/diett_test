...
$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    if(isset($_POST['action']) && $_POST['action']=="selectmeal"){
        $UserID  = $_POST['UserID'];
        $mealID  = $_POST['mealID'];

        
    	           
               $sql = "INSERT INTO DailyMeal (daily_id, mealID,UserID) VALUES (?,?,?)";
            $stmt = $db->prepare($sql);
            if($stmt->execute([uniqid(),$mealID, $UserID])){
                    $status=true;
                    $message="successful Add new Meal";
